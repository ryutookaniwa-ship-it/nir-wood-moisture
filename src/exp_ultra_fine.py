"""Ultra-fine grid around new best: ps=9, lv=35 → RMSE=15.1704
Explore lv={27,29,31,33,35,37} x ps={7,8,9,10,11}
Hypothesis: leaves↓ trend continues but will plateau — find the optimum.
"""
import sys
...
for ps in [7, 8, 9, 10, 11]:
    for lv in [23, 27, 29, 31, 33, 35, 37]:
        configs.append({"ps": ps, "lv": lv})
...
for cfg in configs:
    ...
    tag = " ***" if rmse < 15.1704 else (" **" if rmse < 15.2250 else "")
    print(f"ps={cfg['ps']:2d} lv={cfg['lv']:2d}  RMSE={rmse:.4f}{tag}")

print(f"\nBest: ps={best_cfg['ps']} lv={best_cfg['lv']}  RMSE={best_rmse:.4f}")

if best_test is not None:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    sub = pd.DataFrame({"id": test_ids, "pred": np.clip(best_test, 0, None)})
    sub.to_csv(OUTPUT_DIR / "submission_ultra_fine.csv", index=False, header=False)
    print(f"Saved: {OUTPUT_DIR / 'submission_ultra_fine.csv'}")
