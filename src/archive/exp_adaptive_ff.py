"""Experiment: Per-fold adaptive feature fraction
Hypothesis: ff=0.07 is globally optimal, but some LOSO folds (especially sp15)
may benefit from different ff values. For sp15 fold (high-MC extrapolation),
a higher ff might help by using more features for the extreme range.
We dynamically set ff based on the validation species' characteristics.
Actually, a simpler idea: test ff slightly different values very precisely.
I1 tested ff=0.02-0.10 and found 0.07 optimal. But did they test 0.06 and 0.08?
Let's do a fine grid: 0.05, 0.06, 0.07, 0.08, 0.09 to see if there's
a slightly better sweet spot.
"""
import numpy as np
import pandas as pd
...
# Fine grid search for ff
for ff in [0.05, 0.06, 0.065, 0.07, 0.075, 0.08, 0.09]:
    LGBM_PARAMS = dict(
    ...
    print(f"ff={ff:.3f}  RMSE={rmse:.4f}  avg_rounds={int(np.mean(best_rounds))}")

    if ff == 0.07:
        test_original_07 = np.power(np.clip(test_preds, 0, None), 1.0 / POWER)

# Save P1 baseline submission for comparison
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
sub = pd.DataFrame({"id": test_ids, "pred": np.clip(test_original_07, 0, None)})
sub.to_csv(OUTPUT_DIR / "submission_exp_adaptive_ff.csv", index=False, header=False)
print(f"\nSaved P1 (ff=0.07) submission")
