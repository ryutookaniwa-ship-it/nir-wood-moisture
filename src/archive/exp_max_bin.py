"""Experiment: max_bin tuning with P1 params
Q2 tested max_bin=127 but changed leaves/mcs simultaneously.
max_bin alone with P1-exact params hasn't been tested.
Lower max_bin = coarser feature discretization = stronger regularization.
Default is 255. Testing: 63, 127, 255, 511.
"""
import numpy as np
import pandas as pd
...
for max_bin in [63, 127, 255, 511]:
    ...
    print(f"max_bin={max_bin:4d}  RMSE={rmse:.4f}  avg_rounds={int(np.mean(best_rounds))}")

print(f"\nP1 reference (max_bin=255): RMSE=15.4725")
