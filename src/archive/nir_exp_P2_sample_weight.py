"""
Experiment P2: 高MC域 sample_weight
======================================
sp15(RMSE=39.20, bias=-6.80, MC_max=298.6%)のボトルネックは高MC外挿不足。
MC > threshold のサンプルを重み付けして高MC域の学習を強化。
p=0.30変換と組み合わせ。

探索:
  threshold: [50, 100, 150] %
  weight_high: [1.5, 2.0, 3.0, 4.0]
...
print(f"=== Experiment {EXP}: sample_weight 高MC域重み付け ===")
print(f"M2ベース(p=0.30, LOSO=15.5877, LB=15.545)")
print(f"高MC割合: MC>50%={( y_train>50).mean():.1%}, MC>100%={(y_train>100).mean():.1%}, MC>150%={(y_train>150).mean():.1%}\n")
print(f"{'thresh':>8}  {'w_high':>7}  {'LOSO':>8}  {'vs M2':>7}")
print("-" * 38)

best_rmse = np.inf; best_cfg = None; best_iters = None

for thresh in [50, 100, 150]:
    for w_high in [1.5, 2.0, 3.0, 4.0]:
...
print(f"\nBest: thresh={best_cfg[0]}%, w={best_cfg[1]}  LOSO={best_rmse:.4f}  vs M2: {best_rmse - 15.5877:+.4f}")

if best_rmse < 15.5877:
    thresh_b, w_b = best_cfg
    weights_full = np.where(y_train > thresh_b, w_b, 1.0)
    y_trans_full = y_train ** POWER
    dtrain_f = lgb.Dataset(Xtr, label=y_trans_full, weight=weights_full)
    ...
    submit_to_signate(OUT, f"{EXP}: weight(MC>{thresh_b}%)={w_b} LOSO={best_rmse:.4f}", loso=best_rmse)
else:
    print("\n[Skip] M2を超えなかった → 提出なし")
