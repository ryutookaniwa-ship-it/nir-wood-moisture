"""Test early stopping patience with best params (ps=10, lv=47).
Previous test with P1 params showed patience=100 gave trivial improvement.
But with ps+lv changes, the optimal patience may differ.
Also test bagging (subsample) which can reduce overfitting.
"""
import sys
...
configs = [
    # Patience experiments
    {"label": "patience=50 (baseline)", "patience": 50, "subsample": 1.0, "subsample_freq": 0},
    {"label": "patience=75", "patience": 75, "subsample": 1.0, "subsample_freq": 0},
    {"label": "patience=100", "patience": 100, "subsample": 1.0, "subsample_freq": 0},
    {"label": "patience=150", "patience": 150, "subsample": 1.0, "subsample_freq": 0},
    # Bagging experiments (with patience=50)
    {"label": "bag=0.8 freq=5", "patience": 50, "subsample": 0.8, "subsample_freq": 5},
    {"label": "bag=0.7 freq=5", "patience": 50, "subsample": 0.7, "subsample_freq": 5},
    {"label": "bag=0.9 freq=3", "patience": 50, "subsample": 0.9, "subsample_freq": 3},
    # lambda_l1/l2 regularization
    {"label": "l1=0.1", "patience": 50, "subsample": 1.0, "subsample_freq": 0, "lambda_l1": 0.1},
    {"label": "l2=0.1", "patience": 50, "subsample": 1.0, "subsample_freq": 0, "lambda_l2": 0.1},
    {"label": "l1=1.0", "patience": 50, "subsample": 1.0, "subsample_freq": 0, "lambda_l1": 1.0},
    {"label": "l2=1.0", "patience": 50, "subsample": 1.0, "subsample_freq": 0, "lambda_l2": 1.0},
]
...
for cfg in configs:
    LGBM_PARAMS = dict(
        ...
        random_state=42, learning_rate=0.02, num_leaves=47,
    ...
    if "lambda_l1" in cfg:
        LGBM_PARAMS["lambda_l1"] = cfg["lambda_l1"]
    if "lambda_l2" in cfg:
        LGBM_PARAMS["lambda_l2"] = cfg["lambda_l2"]
    ...
    print(f"{cfg['label']:25s}  RMSE={rmse:.4f}{tag}")
