"""
Experiment P3: DART booster + p=0.30
=======================================
DARTのdropout regularizationがfeature_fraction=0.07との相乗効果で
種固有パターン依存をさらに分散できるか検証。
DART: 各ツリーをランダムにdropしてブースティングの過学習を抑制。

探索:
  rate_drop: [0.05, 0.10, 0.15, 0.20]  (dropされるツリーの割合)
  skip_drop: 0.5 固定
固定rounds: 800 (early_stoppingはDARTで不安定なため固定)
ベース: M2 (p=0.30, LOSO=15.5877, LB=15.545)
"""
import sys
import numpy as np
from sklearn.decomposition import PCA
sys.path.insert(0, r"C:\Users\ryuch\OneDrive\デスクトップ\my_kaggle_project\src")
sys.path.insert(0, r"C:\Users\ryuch\OneDrive\デスクトップ\my_kaggle_project\src\nir")
from nir_loso_utils import (
    load_data, msc, sg_deriv, loso_folds, loso_rmse,
    save_submission, submit_to_signate, LGBM_BASE_PARAMS,
)
import lightgbm as lgb
import warnings; warnings.filterwarnings("ignore")

EXP = "P3"
OUT_DIR = r"C:\Users\ryuch\OneDrive\デスクトップ\my_kaggle_project\output\nir-wood-moisture"
FIXED_ROUNDS = 800

def compute_epo_matrix(X, y, sp, bin_width=10.0, n_components=5, min_species=2):
    bins = np.arange(0, y.max() + bin_width, bin_width)
    all_dirs = []
    for lo in bins[:-1]:
        hi = lo + bin_width; mask = (y >= lo) & (y < hi)
        if mask.sum() < 4: continue
        sp_in = np.unique(sp[mask])
        if len(sp_in) < min_species: continue
        sp_means = np.array([X[mask][sp[mask] == s].mean(axis=0) for s in sp_in])
        inter = sp_means - sp_means.mean(axis=0)
        n_c = min(n_components, inter.shape[0] - 1)
        if n_c < 1: continue
        pca = PCA(n_components=n_c, random_state=42); pca.fit(inter)
        all_dirs.append(pca.components_)
    if not all_dirs: return np.zeros((X.shape[1], 1))
    D = np.vstack(all_dirs); _, _, Vt = np.linalg.svd(D, full_matrices=False)
    return Vt[:n_components].T

def apply_epo(X, V): return X - (X @ V) @ V.T

data = load_data()
y_train = data["y_train"]; X_train_raw = data["X_train_raw"]
X_test_raw = data["X_test_raw"]; test_ids = data["test_ids"]; sp_train = data["sp_train"]

ref    = X_train_raw.mean(axis=0)
Xtr_sg = sg_deriv(msc(X_train_raw, ref), window=9, polyorder=2)
Xte_sg = sg_deriv(msc(X_test_raw,  ref), window=9, polyorder=2)
V      = compute_epo_matrix(Xtr_sg, y_train, sp_train, n_components=5)
Xtr    = apply_epo(Xtr_sg, V)
Xte    = apply_epo(Xte_sg, V)

POWER = 0.30
y_trans = y_train ** POWER
inv = lambda pred: np.clip(pred, 0, None) ** (1.0 / POWER)

base_params = {**LGBM_BASE_PARAMS,
               "boosting_type": "dart",
               "learning_rate": 0.02, "num_leaves": 63,
               "feature_fraction": 0.07, "min_child_samples": 10,
               "skip_drop": 0.5}

print(f"=== Experiment {EXP}: DART booster (p=0.30, fixed_rounds={FIXED_ROUNDS}) ===")
print(f"M2ベース(LOSO=15.5877, LB=15.545)\n")
print(f"{'rate_drop':>10}  {'LOSO':>8}  {'vs M2':>7}")
print("-" * 32)

best_rmse = np.inf; best_rd = None

for rate_drop in [0.05, 0.10, 0.15, 0.20]:
    params = {**base_params, "rate_drop": rate_drop}

    oof_trans = np.zeros(len(y_train))
    for tr_idx, va_idx, _ in loso_folds(sp_train):
        dtrain = lgb.Dataset(Xtr[tr_idx], label=y_trans[tr_idx])
        m = lgb.train(params, dtrain, num_boost_round=FIXED_ROUNDS,
                      callbacks=[lgb.log_evaluation(-1)])
        oof_trans[va_idx] = m.predict(Xtr[va_idx])

    oof  = inv(oof_trans)
    rmse = loso_rmse(oof, y_train)
    diff = rmse - 15.5877
    flag = " <-- best" if rmse < best_rmse else ""
    print(f"  rate_drop={rate_drop:.2f}  {rmse:8.4f}  {diff:+7.4f}{flag}")

    if rmse < best_rmse:
        best_rmse = rmse; best_rd = rate_drop

print(f"\nBest: rate_drop={best_rd}  LOSO={best_rmse:.4f}  vs M2: {best_rmse - 15.5877:+.4f}")

if best_rmse < 15.5877:
    final_params = {**base_params, "rate_drop": best_rd}
    dtrain_f = lgb.Dataset(Xtr, label=y_train ** POWER)
    final = lgb.train(final_params, dtrain_f, num_boost_round=FIXED_ROUNDS,
                      callbacks=[lgb.log_evaluation(-1)])
    preds = inv(final.predict(Xte))
    OUT = f"{OUT_DIR}/submission_{EXP}_rd{int(best_rd*100)}.csv"
    save_submission(test_ids, preds, OUT)
    submit_to_signate(OUT, f"{EXP}: DART rate_drop={best_rd} LOSO={best_rmse:.4f}", loso=best_rmse)
else:
    print("\n[Skip] M2を超えなかった → 提出なし")
