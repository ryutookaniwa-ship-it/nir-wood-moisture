"""Experiment: max_bin=127 with P1 pipeline — full run with submission
max_bin=127 showed LOSO-RMSE=15.4691 (-0.0034 vs P1).
Generate full submission and also test max_bin=127 with slight patience increase.
"""
import numpy as np
import pandas as pd
...
for tr_idx, va_idx, sp_id in loso_folds(sp_train):
    ...
    oof_sp = np.power(np.clip(oof[va_idx], 0, None), 1.0 / POWER)
    y_sp = y_train[va_idx]
    sp_rmse = float(np.sqrt(np.mean((y_sp - oof_sp) ** 2)))
    sp_rmses[sp_id] = sp_rmse
    print(f"  sp={sp_id:2d}: RMSE={sp_rmse:.2f} (n={len(va_idx)}, rounds={model.best_iteration})")
...
print(f"\nPer-species RMSE breakdown:")
for sp_id in sorted(sp_rmses.keys()):
    print(f"  sp{sp_id:2d}: {sp_rmses[sp_id]:.2f}")
...
print(f"\nSaved: {OUTPUT_DIR / f'submission_{EXP_ID}.csv'}")
