"""Experiment: Robust EPO with median-based species means
Hypothesis: Standard EPO computes species means (arithmetic), which are
sensitive to outliers (especially in species with high-MC samples like sp15).
Using median instead of mean for species-level aggregation in EPO should
produce more robust inter-species directions, potentially improving
generalization to unseen test species.
"""
import numpy as np
import pandas as pd
...
def compute_epo_matrix_robust(X, y, sp, bin_width=10.0, n_components=5, min_species=2):
    """EPO with median-based species centroids instead of mean."""
    ...
    for lo in bins[:-1]:
        ...
        sp_medians = np.array([np.median(X[mask][sp[mask] == s], axis=0) for s in sp_in])
        inter = sp_medians - sp_medians.mean(axis=0)
...
V_epo = compute_epo_matrix_robust(X_tr_sg, y_train, sp_train, bin_width=10.0, n_components=5)
