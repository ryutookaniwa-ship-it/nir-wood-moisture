"""Q2 revisit: mcs=30 was rejected due to LB degradation.
Discord insight: LOSO is the true metric (Public/Private split by species).
Hypothesis: mcs=30 + path_smooth=10 could compound — both are regularization
but on different axes (mcs: leaf sample threshold, ps: leaf value smoothing).
Q2 original: lv=47+mcs=30+mb=127 → LOSO=15.2130, LB=15.535
Our best: ps=10+lv=47+mcs=10 → LOSO=15.2356
Target: beat 15.2130 by adding ps to Q2's params (without mb=127 which hurt).
"""
import sys
...
OUTPUT_DIR = Path(r"C:\Users\ryuch\OneDrive\デスクトップ\Ryuto\4_ds\competitions\signate\nir_wood_moisture\output")
...
configs = [
    # New best baseline
    {"label": "ps7+lv33 (new best)", "lv": 33, "mcs": 10, "ps": 7},
    # mcs exploration at new best
    {"label": "ps7+lv33+mcs5", "lv": 33, "mcs": 5, "ps": 7},
    {"label": "ps7+lv33+mcs15", "lv": 33, "mcs": 15, "ps": 7},
    {"label": "ps7+lv33+mcs20", "lv": 33, "mcs": 20, "ps": 7},
    {"label": "ps7+lv33+mcs30", "lv": 33, "mcs": 30, "ps": 7},
    # Q2 revisit at new optimal lv
    {"label": "ps7+lv33+mcs30 (Q2 style)", "lv": 33, "mcs": 30, "ps": 7},
    {"label": "ps9+lv35+mcs30", "lv": 35, "mcs": 30, "ps": 9},
    {"label": "ps11+lv31+mcs30", "lv": 31, "mcs": 30, "ps": 11},
    # Top configs with mcs=15
    {"label": "ps9+lv35+mcs15", "lv": 35, "mcs": 15, "ps": 9},
    {"label": "ps11+lv31+mcs15", "lv": 31, "mcs": 15, "ps": 11},
    {"label": "ps8+lv31+mcs15", "lv": 31, "mcs": 15, "ps": 8},
]
...
for cfg in configs:
    LGBM_PARAMS = dict(
        ...
        random_state=42, learning_rate=0.02, num_leaves=cfg["lv"],
    ...
    if rmse < best_rmse:
        ...
        best_test = test_original.copy()
    tag = " ***" if rmse < 15.2130 else (" **" if rmse < 15.2356 else "")
    print(f"{cfg['label']:35s}  RMSE={rmse:.4f}{tag}")
...
if best_test is not None:
    ...
    sub = pd.DataFrame({"id": test_ids, "pred": np.clip(best_test, 0, None)})
    sub.to_csv(OUTPUT_DIR / "submission_q2_revisit.csv", index=False, header=False)
    print(f"Saved: {OUTPUT_DIR / 'submission_q2_revisit.csv'}")
