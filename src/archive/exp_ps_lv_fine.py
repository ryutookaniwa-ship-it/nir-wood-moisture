"""Fine grid: path_smooth x leaves around best (ps=10, lv=47)
Best so far: ps=10+lv=47 = 15.2356
Also test ps=9 (best in single-param grid) with lv=47.
"""
import sys
...
# Fine grid around the best: ps=10, lv=47
configs = []
for ps in [7, 8, 9, 10, 11, 12]:
    for lv in [39, 43, 47, 51, 55]:
...
for cfg in configs:
    ...
    tag = " ***" if rmse < 15.2356 else (" **" if rmse < 15.37 else "")
...
if best_test is not None:
    ...
    sub.to_csv(OUTPUT_DIR / "submission_ps_lv_fine.csv", index=False, header=False)
    print(f"Saved: {OUTPUT_DIR / 'submission_ps_lv_fine.csv'}")
