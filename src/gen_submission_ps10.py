"""Generate submission: P1 + path_smooth=10 only (leaves=63, mcs=10, max_bin=255)
Safe option: keeps P1 structure, adds only the best single-parameter improvement.
LOSO-RMSE = 15.3723 (P1: 15.4725, delta = -0.100)
"""
import sys
...
LGBM_PARAMS = dict(
    ...
    path_smooth=10,  # Only change from P1
)
...
sub.to_csv(OUTPUT_DIR / "submission_T2_ps10.csv", index=False, header=False)
print(f"Saved: {OUTPUT_DIR / 'submission_T2_ps10.csv'}")
