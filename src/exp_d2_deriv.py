"""Experiment: SG 2nd derivative (d=2) with P1 pipeline
Hypothesis: 2nd derivative removes both baseline offset and linear slope,
isolating absorption peak curvature. This changes what EPO removes.
d=2 is standard in NIR chemometrics but has never been tested alone with EPO.
V4c tried d=1+d=2 concat (+0.39 worse), V4d tried d=0 (catastrophic),
but d=2 alone is untested.
"""
import numpy as np
import pandas as pd
...
# Preprocessing: MSC -> SG 2nd derivative (d=2) -> EPO
X_tr_msc, msc_ref = msc(X_train_raw)
X_te_msc, _ = msc(X_test_raw, reference=msc_ref)

X_tr_sg = sg_deriv(X_tr_msc, window=9, polyorder=2, deriv=2)  # d=2 instead of d=1
X_te_sg = sg_deriv(X_te_msc, window=9, polyorder=2, deriv=2)
