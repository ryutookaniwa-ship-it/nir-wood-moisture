"""Experiment: min_gain_to_split tuning
This LGBM parameter controls the minimum gain required to make a split.
It hasn't been explored in any previous experiment. Default is 0.0.
A small positive value acts as tree-level regularization, different from
feature_fraction or min_child_samples.
Also test path_smooth (smoothing for leaf predictions) which is
another unexplored regularization parameter.
"""
import numpy as np
import pandas as pd
...
configs = [
    {"min_gain": 0.0, "path_smooth": 0, "label": "P1 baseline"},
    {"min_gain": 0.001, "path_smooth": 0, "label": "mg=0.001"},
    {"min_gain": 0.01, "path_smooth": 0, "label": "mg=0.01"},
    {"min_gain": 0.1, "path_smooth": 0, "label": "mg=0.1"},
    {"min_gain": 0.5, "path_smooth": 0, "label": "mg=0.5"},
    {"min_gain": 0.0, "path_smooth": 1, "label": "ps=1"},
    {"min_gain": 0.0, "path_smooth": 10, "label": "ps=10"},
    {"min_gain": 0.0, "path_smooth": 50, "label": "ps=50"},
    {"min_gain": 0.01, "path_smooth": 10, "label": "mg=0.01+ps=10"},
]
...
for cfg in configs:
    LGBM_PARAMS = dict(
        ...
        min_gain_to_split=cfg["min_gain"],
    ...
    print(f"{cfg['label']:20s}  RMSE={rmse:.4f}  avg_rounds={int(np.mean(best_rounds))}")
